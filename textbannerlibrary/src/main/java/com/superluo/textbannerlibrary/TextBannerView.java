...
public class TextBannerView extends RelativeLayout {
    ...
    /**设置数据集合*/
    public void setDatasByHtml(List<String> datas){
        this.mDatas = datas;
        if (DisplayUtils.notEmpty(mDatas)){
            mViewFlipper.removeAllViews();
            for (int i = 0; i < mDatas.size(); i++) {
                TextView textView = new TextView(getContext());
                setTextViewByHtml(textView,i);
    ...
    /**设置TextView*/
    private void setTextViewByHtml(TextView textView,int position){
        textView.setText(Html.fromHtml(mDatas.get(position)));
        //任意设置你的文字样式，在这里
        textView.setSingleLine(isSingleLine);
        textView.setEllipsize(TextUtils.TruncateAt.END);
        textView.setTextColor(mTextColor);
